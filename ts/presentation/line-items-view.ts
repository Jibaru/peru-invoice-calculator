...
export const RenderLineItem = (lineItem: LineItem) => {
  ...
  td.textContent = `${lineItem.unitPrice.toFixed(
    4
  )} (${lineItem.startingUnitPrice.toFixed(4)})`;
  tr.appendChild(td);
  ...
  td.textContent = `${lineItem.igvTotal.toFixed(
    4
  )} (${lineItem.igvPercentage.toFixed(4)}%)`;
  tr.appendChild(td);

  td = document.createElement("td");
  td.textContent = `${lineItem.iscTotal.toFixed(
    4
  )} (${lineItem.iscPercentage.toFixed(4)}%)`;
  tr.appendChild(td);

  td = document.createElement("td");
  td.textContent = `${lineItem.rcTotal.toFixed(
    4
  )} (${lineItem.rcPercentage.toFixed(4)}%)`;
  tr.appendChild(td);

  td = document.createElement("td");
  td.textContent = `${lineItem.discount.toFixed(
    4
  )} (${lineItem.startingDiscount.toFixed(4)})`;
  tr.appendChild(td);
...
export const ReRenderLineItems = (lineItems: LineItem[]) => {
  lineRows.innerHTML = "";
